...
@section('content')
        ...
        <div class="row" id="filters-wrapper">
            <div class="col-xs-6 col-xs-offset-3">
                ...
                @foreach ($aggregations['aggregations']['price_ranges']['buckets'] as $bucket)
                    <a href="?query={{ $query }}&page={{ $page }}&startprice={{ $bucket['from'] }}&endprice={{ $bucket['to'] }}&status={{ $status ?? '' }}&category={{ $category ?? '' }}" class="{{ $bucket['from'] == $startPrice && $bucket['to'] == $endPrice ? 'active' : '' }}">
                ...
                @foreach ($aggregations['aggregations']['statuses']['buckets'] as $bucket)
                    <a href="?query={{ $query }}&page={{ $page }}&status={{ urlencode($bucket['key']) }}&startprice={{ $startPrice?? '' }}&endprice={{ $endPrice ?? '' }}&category={{ $category ?? '' }}" class="{{ $bucket['key'] == $status ? 'active' : '' }}">
                ...
                @foreach ($aggregations['aggregations']['categories']['categories_count']['buckets'] as $bucket)
                    <a href="?query={{ $query }}&page={{ $page }}&category={{ urlencode($bucket['key']) }}&status={{ $status ?? '' }}&startprice={{ $startPrice ?? '' }}&endprice={{ $endPrice ?? '' }}" class="{{ $bucket['key'] == $category ? 'active' : '' }}">
                        {{ ucfirst($bucket['key']) }} ({{ $bucket['doc_count'] }})
                    </a>
                @endforeach
            </div>
    ...
    @if (!empty($hits))
                ...
                <div class="col-xs-8 col-xs-offset-2">
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            <a href="/product/view/{{ $hit['_id'] }}">{{ $hit['_source']['name'] }}</a></div>
        ...
        <div class="row">
            <div class="pagination-wrapper col-xs-8 col-xs-offset-2 text-center">
                <nav>
                    <ul class="pagination">
                        <li>
                        ...
                        @for ($i = 1; $i <= $maxPagesNum; $i++)
                            <li {!! $i == $page ? 'class="active"' : '' !!}><a href="?query={{ urlencode($query) }}&page={{ $i }}&status={{ $status??'' }}&startprice={{ $startPrice?? '' }}&endprice={{ $endPrice??'' }}&category={{ $category??'' }}">{{ $i }}</a></li>
                        @endfor
